task_info = {
    "task_id": "fcf7dea4-61b7-4ec1-a1c6-c8262a907188",
...
def run_task():
    ...
    try:
        ...
        log_thought("task_mock", {"task": "必要なライブラリをインポートする"})
        add_insight("モック: 必要なライブラリをインポートする の初期分析", 0.7)
        result = "モック実行: 必要なライブラリをインポートする が完了しました"
        if result is None:
